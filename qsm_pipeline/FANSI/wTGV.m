function out = wTGV(params,varargin)
...
[phase, alpha1, alpha0, N, Kernel, mu1, mu0, mu2, maxOuterIter, ...
 tolUpdate, regweight, W, isGPU,isPrecond, isVoutput] = parse_inputs(params, varargin{:});
    
    
    % Redefinition of variable for computational efficiency
    FWy = fftn(W.*phase./(W+mu2));
...
if isGPU 
    ...
    Et1 = conj(E1);     Et2 = conj(E2);     Et3 = conj(E3);     Kt = conj(Kernel);
    ...
    for t = 1:maxOuterIter
        ...
        z2 = FWy + mu2*fftn( real(ifftn(Kernel.*Fx+s2))./(W + mu2) );
...
function [phase, alpha1, alpha0, N, Kernel, mu1, mu0, mu2, maxOuterIter, tolUpdate, regweight, W, isGPU, isPrecond, isVoutput] = parse_inputs(params, varargin)
global DEBUG; if isempty(DEBUG); DEBUG = false; end
