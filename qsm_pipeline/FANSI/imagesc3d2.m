...
%
% Use example:
% imagesc3d2(chi_cosmos, size(chi_cosmos)/2, 1, [90,90,-90], [-0.10,0.14], [], 'COSMOS')
%
...
if ~MIP
    if axis_square
        ...
        figure(fig_num), subplot(132), 
        imagesc( imrotate(squeeze(mean(img(:,pos(2)-Lavg_size:pos(2)+Ravg_size,:),2)), rot_deg(2)), scale_fig ), axis image off, colormap gray
            title(title_fig, 'color', 'w', 'fontsize', 24)
