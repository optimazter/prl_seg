function [chi, Bnuc] = chi_intcomplexsphere(FOV,N,center,radius, xin, xout,phs_scale)
...
% defined voxels in the complex domain (unit magnitude). 
% This provides a test of intra voxel dephasing effects based on the 
% acquisition model on MRI devises.
%
% Parameters:
...
for i=1:N(1)
    for j=1:N(2)
        for k=1:N(3)
            ...
            dBnuc = (1-dchi*2/3);
            dsignal = exp(1i*dBmac.*dBnuc*phs_scale);
            Bnuc(i,j,k) = angle(mean(dsignal(:)))/phs_scale;
            end
        end
    end
end
