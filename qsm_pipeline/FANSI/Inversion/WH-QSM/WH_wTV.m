...
% params.isPrecond: preconditionate solution by smart initialization
...
N = size(params.input);
    ...
    if isPrecond
        z2 = Wy;
        local = msmv_noextrapolation( params.input, mask, [1 1 1], 6, false );
        phi_h = params.input-local;
        phi_h = extrapolatefield_iter( phi_h, mask, 1, 100 );
    else
        z2 = zeros(N,'single');
    end
    s2 = zeros(N,'single');
...
for t = 1:num_iter
    ...
    if t < num_iter
        ...
        z2 = Wy + mu2*real(ifftn(kernel.*Fx)+s2+phi_h )./(W + mu2);
               
        
        phi_h = real(ifftn( (muh .* conj(EE2).*fftn(z_h-s_h) + mu2*fftn(z2-s2) - mu2*kernel.*Fx) ./ (eps + mu2 + muh .* EE2.*conj(EE2)) ));
        Fphi_h = fftn(phi_h);
