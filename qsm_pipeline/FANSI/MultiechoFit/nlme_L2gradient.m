function out = nlme_L2gradient(params)
% Nonlinear multi-echo fit with Tikhonov regularization, Gradient domain. ADMM solver.
%
...
for t = 1:num_iter
    ...
    F2 = zeros( N );
    for tt = 1:nE
        F1 = F1 + TE(tt)*fftn(z(:,:,:,tt)-s(:,:,:,tt));
        F2 = F2 + fftn(z(:,:,:,tt)-s(:,:,:,tt));
    end
    % b and theta are solved jointly
    b = real(ifftn((A4.*F1-A(2)*F2)./dA));
    theta = real(ifftn((A(1)*F2-A(3)*F1)./dA));
