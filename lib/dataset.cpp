...
void createLesionDataset(const fs::path &loadDir, const fs::path &saveDir, const std::string &imgFileName, const std::string &allLesionsFileName, const std::string &PRLFileName, const std::vector<float> trainValTestSplit)
{
    ...
    {
        fs::path imgPath = loadDir / patientEntry / imgFileName;
        fs::path allLesionsPath = loadDir / patientEntry / allLesionsFileName;
        fs::path PRLPath = loadDir / patientEntry / PRLFileName;

        if (!(fs::is_regular_file(imgPath) && fs::is_regular_file(allLesionsPath) && fs::is_regular_file(PRLPath)))
        {
            std::cout << patientEntry << " is not a valid entry!\n";
            continue;
        }

        images.push_back(NIfTIToTensor(imgPath, sitk::sitkFloat32));
        allLesions.push_back(NIfTIToTensor(allLesionsPath, sitk::sitkUInt8));
        PRLs.push_back(NIfTIToTensor(PRLPath, sitk::sitkUInt8));

        std::cout << "\rA total of: " << i++ << " patients have been loaded.";
        std::cout.flush(); 
    }
    std::cout << std::endl;

    torch::Tensor imagesTen = torch::cat(images);
    torch::Tensor allLesionsTen = torch::cat(allLesions);
    torch::Tensor PRLsTen = torch::cat(PRLs);
    torch::Tensor labelsTen;

    assert(imagesTen.sizes() == allLesionsTen.sizes() && imagesTen.sizes() == PRLsTen.sizes());

    auto shape = imagesTen.sizes();
    ...
    auto imagesTenCropped = imagesTen.slice(DIM_H, H - W, H);
    ...
    auto imagesTenFlipped = torch::flip(imagesTenCropped, {DIM_H});
...
{
    torch::Tensor tensor;
    sitk::Image img = sitk::ReadImage(path, dtype);
    ...
    else if (img.GetPixelIDValue() == sitk::sitkUInt8)
    {
        ...
        auto a = tensor.accessor<u_int8_t, 3>();
