...
def create_star_dataset(
    ...
    """
    Creates a dataset with masked SWI and T2*-phase image paired from the given patient directories.
    The dataset is saved in the specified directories for training/validation and testing.
    Args:
        ...
        save_train_val_dir (str): Directory to save training and validation data.
        save_test_dir (str): Directory to save test data.
        num_test_samples (int): Number of samples to reserve for testing.
        crop (SpatialCrop): SpatialCrop object to crop the images.
        high_pass_kernel_size (int): Kernel size for high pass filtering the T2* phase image.
        augment_prl (bool): Whether to augment PRL images or not to balance data size.
    """
...
def augment_prl_images(train_img: Union[torch.Tensor, List[torch.Tensor]], train_label: torch.Tensor, progress: Progress = None) -> Tuple[list, list]:
    ...
    """
    Augments PRL images by applying random rotations, flips, and Gaussian noise.
    Args:
        train_img (Union[torch.Tensor, List[torch.Tensor]]): The training images to augment.
        train_label (torch.Tensor): The labels corresponding to the training images.
        progress (Progress, optional): A Progress object for tracking the augmentation process.
    Returns:
        Tuple[list, list]: A tuple containing the augmented images and labels.
    """
...
def augment_image(image: torch.Tensor, mask: torch.Tensor, prob: float = 1.0, progress: Progress = None) -> List[Tuple[torch.Tensor, torch.Tensor]]:
    ...
    """
    Augments a single image and its corresponding mask by applying random rotations, flips, Gaussian noise, and affine transformations.
    Args:
        image (torch.Tensor): The image to augment.
        mask (torch.Tensor): The mask corresponding to the image.
        prob (float): Probability of applying the augmentation. Default is 1.0 (always apply).
        progress (Progress, optional): A Progress object for tracking the augmentation process.
    Returns:
        List[Tuple[torch.Tensor, torch.Tensor]]: A list of tuples containing the augmented images and masks.
    """
...
def augment_dataset(images: torch.Tensor, labels = torch.Tensor, progress = None):
    """
    Augments a dataset of images and labels by applying random rotations, flips, Gaussian noise, and affine transformations.
    Args:
        images (torch.Tensor): The images to augment, shape should be (N, C, D, H, W).
        labels (torch.Tensor): The labels corresponding to the images, shape should be (N, C, D, H, W).
        progress (Progress, optional): A Progress object for tracking the augmentation process.
    Returns:
        Tuple[torch.Tensor, torch.Tensor]: A tuple containing the augmented images and labels.
    """
...
def save_lazy_dataset(train_imgs, train_labels, val_imgs, val_labels, test_imgs, test_labels, save_train_dir, save_val_dir, save_test_dir):
    """
    Saves the training, validation, and test datasets to the specified directories in a lazy format.
    Args:
        train_imgs (torch.Tensor): Training images.
        train_labels (torch.Tensor): Training labels.
        val_imgs (torch.Tensor): Validation images.
        val_labels (torch.Tensor): Validation labels.
        test_imgs (torch.Tensor): Test images.
        test_labels (torch.Tensor): Test labels.
        save_train_dir (str): Directory to save training data.
        save_val_dir (str): Directory to save validation data.
        save_test_dir (str): Directory to save test data.
    """
...
def create_star_dataset_3d(
    ...
    """
    Creates a 3D dataset with masked SWI and T2*-phase image paired from the given patient directories.
    The dataset is saved in the specified directories for training, validation, and testing.
    Args:
        patient_dirs (List[str]): List of directories containing patient data.
        flair_fname (str): Filename of the FLAIR image.
        ...
        save_val_dir (str): Directory to save validation data.
        save_test_dir (str): Directory to save test data.
        num_val_samples (int): Number of samples to reserve for validation.
        num_test_samples (int): Number of samples to reserve for testing.
        sub_voxel_size (list): Size of the sub-voxel crops to create from the images.
    """
